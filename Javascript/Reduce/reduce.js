...
*/
var avgAge = persons.reduce((x,y,z)=>{
	let total = x+y.age;

	if (z===persons.length-1) 
	{
...
function longestName(array) {
	...
	var longName = array.reduce((x,y,z)=>{

	let lengthName = y.name.first.length + y.name.last.length;


		if (lengthName>x.length-1) 
		{
...
function repeatChar(str,char) {
	var chrachterArray = str.split('');
	var strings=[];

	
	var result=chrachterArray.reduce((x,y)=>{
